# 你好！👋 我是阮思康

*其他语言版本: [English](README.md), [日本語](README.ja-JP.md), [廣東話（香港）](README.zh-HK.md), [繁體中文（台灣省）](README.zh-TW.md)*

拥有企业系统、Web应用和AI基础设施经验的软件开发者。

- BA (Hons) AHCC（应用及人本计算学（荣誉）文学士），现称应用计算（AC），经认证等同于应用计算理学学士
- 香港恒生大学数据科学与人工智能硕士

## 🔧 技术与工具
![C#](https://img.shields.io/badge/-C%23-239120?style=flat-square&logo=c-sharp&logoColor=white)
![ASP.Net](https://img.shields.io/badge/-ASP.Net-512BD4?style=flat-square&logo=.net&logoColor=white)
![Node.js](https://img.shields.io/badge/-Node.js-339933?style=flat-square&logo=node.js&logoColor=white)
![Express.js](https://img.shields.io/badge/-Express.js-000000?style=flat-square&logo=express&logoColor=white)
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)
![MS SQL Server](https://img.shields.io/badge/-MS%20SQL%20Server-CC2927?style=flat-square&logo=microsoft-sql-server&logoColor=white)
![MySQL](https://img.shields.io/badge/-MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/-PostgreSQL-336791?style=flat-square&logo=postgresql&logoColor=white)
![React](https://img.shields.io/badge/-React-61DAFB?style=flat-square&logo=react&logoColor=black)
![Next.js](https://img.shields.io/badge/-Next.js-000000?style=flat-square&logo=next.js&logoColor=white)
![Vue.js](https://img.shields.io/badge/-Vue.js-4FC08D?style=flat-square&logo=vue.js&logoColor=white)
![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![HTML5](https://img.shields.io/badge/-HTML5-E34F26?style=flat-square&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/-CSS3-1572B6?style=flat-square&logo=css3&logoColor=white)
![PyTorch](https://img.shields.io/badge/-PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white)
![LangChain](https://img.shields.io/badge/-LangChain-1C3C3C?style=flat-square&logo=langchain&logoColor=white)
![vLLM](https://img.shields.io/badge/-vLLM-000000?style=flat-square&logo=vllm&logoColor=white)
![Ollama](https://img.shields.io/badge/-Ollama-000000?style=flat-square&logo=ollama&logoColor=white)
![Power BI](https://img.shields.io/badge/-Power%20BI-F2C811?style=flat-square&logo=power-bi&logoColor=black)
![Docker](https://img.shields.io/badge/-Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![Podman](https://img.shields.io/badge/-Podman-892CA0?style=flat-square&logo=podman&logoColor=white)
![Linux](https://img.shields.io/badge/-Linux-FCC624?style=flat-square&logo=linux&logoColor=black)

**后端与Web**
- C# ASP.Net、Node.js、Express、Python
- MS SQL Server、MySQL、PostgreSQL

**前端**
- React、Next.js、Vue.js、TypeScript、HTML/CSS/JavaScript

**AI与数据**
- PyTorch、LangGraph、LangChain、模型量化 (GGUF/AWQ)
- vLLM、Ollama、本地AI部署
- Power BI (DAX、建模)

**基础设施**
- Docker、Podman、Linux (WSL、Ubuntu)
- Oracle Cloud VPS、NAS、Tailscale

## 💼 工作经验

- **分析师程序员** @ 东华三院 (2024-2025)
  - ASP.Net企业系统开发
  - MS SQL Server数据库设计与优化

- **程序员** @ 政府统计处 (2025)
  - 数据处理与自动化

- **电脑技术员 II** @ 香港中文大学系统工程部 (2024)
  - 前线支持与系统维护
  - 硬件/软件安装

- **Web开发员** @ 机电工程署 (2023)
  - 使用TypeScript将InfluxDB数据转换至TimescaleDB

- **学生导师** @ 香港恒生大学 (2022-2023)
  - 教授编程技能
  - 使用Python、Keras、QT5开发二分类图像识别应用

## 📚 项目

- **本地AI基础设施** (Python、Docker、vLLM)
  - 多GPU推理优化
  - 使用Ollama进行私有LLM部署

- **FOD保费提取API** (TypeScript、Node.js)
  - Web API开发与集成

- **数据分析项目** (Power BI、Python)
  - 决策支持可视化与报告

## 🌱 正在探索

- 高级AI部署技术
- 云原生架构

## 💼 寻求机会

正在寻找以下领域职位：
- 软件开发 / 应用工程
- 数据分析与商业智能
- AI应用开发

## 🎉 有趣的事实

- Minecraft点燃了我对编程的热情；现在我搭建本地AI基础设施而不是红石电路。
- 我在家运行一个私有LLM实验室，进行多GPU推理优化（vLLM、Ollama、RTX 50系列）。
- 动漫与漫画爱好者，偏好日常系、户外系和喜剧类作品。
- 正在学习日本G-Ball（健身球）训练体系，努力获得教练认证。
- 喜欢与志同道合的人讨论IT基础设施、本地AI部署和健身球技巧。

## 💬 联系我

- 邮箱：szehong26@gmail.com
- WhatsApp：+852 67353593
- 微信：yuenszehong
